...
class EnlightenLogger(tf.keras.callbacks.Callback):
    ...
    def on_train_begin(self, logs=None):
        ...
        self.status_bar = self.manager.status_bar(f"Training model '{self.model_name}'", justify=enlighten.Justify.CENTER)
        self.info_bar = self.manager.status_bar('Loss: ??????, Learning Rate: ???????, Batch Size: ???*?????')
        self.steps_bar = self.manager.counter(total=self.config.n_steps, count=self.step_index, desc='Steps', color='green', unit='steps')
