...
def make_val_step_wrapper(
    ...
    loss_fn: Callable,
    metrics: dict[str, MxMetric],
    make_val_step: Callable[..., TrainStepFn],
) -> tft.GenericFunction:
    """
...
@export
def default_metrics(loss_fn, n_steps, n_val_steps) -> dict[str, dict[str, MxMetric]]:
    """
    Default metrics.
    """
    ...
    return {
        ...
        "val_step_metrics": list_to_dict([
            RunningMean(fn=loss_fn, name="eval_loss_epoch", reset_every_epoch=True),
            Rolling(length=n_val_steps, fn=loss_fn, name="eval_loss", reset_every_epoch=False),
        ]),
    }
...
def make_train_loop(
    ...
    if metrics == "default":
        metrics = default_metrics(wrapped_loss_fn, n_steps=n_steps, n_val_steps=n_val_steps)
